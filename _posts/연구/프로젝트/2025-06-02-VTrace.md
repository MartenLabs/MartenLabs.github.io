...
| `--eval`               | MSE, Cosine, STOI 기반의 복원 평가 실행 여부         | 지정 시 `True`                            |
| `--convert_to_mp3`     | wav to mp3 convert 여부                        | 지정 시 `True`                            |
...
폴더 전체 처리, 병렬처리(프로세스 8), 블렌드 알파, 보컬 알파, demucs 생성 음원 저장 안함, MP3 변환, 평가 지표 활성화, GPU 가속(애플 실리콘용 MPS):
